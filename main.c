...
int main() {
    ...
    ASTNode *bool1_for_and = create_bool_node(1);
    ASTNode *bool0 = create_bool_node(0);
    ASTNode *and_op = create_binary_op_node(bool1_for_and, bool0, "&&");

    Value result_and = eval(and_op, env);
    printf("Resultado de 1 && 0: %d\n", result_and.data.i);

    // Teste: operação unária lógica (!1)
    ASTNode *bool1_for_not = create_bool_node(1);
    ASTNode *not_op = create_unary_op_node(bool1_for_not, "!");
